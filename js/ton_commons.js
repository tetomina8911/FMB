...
$(document).ready(function () {
  ...
  $(".common-address .btn-find-address")
    .off()
    .on("click", function () {
...
var defaultAjaxError = function (data) {
  modal.showAlertNoHeaderModal(
    "done",
    { title: "네트워크가 원활하지 않습니다.<br/>잠시 후 다시 시도해 주세요.", btn: "확인" },
    function () {}
  );
};
...
function toast_danger(string) {
  ...
  toast_danger.classList.contains("reveal")
    ? (clearTimeout(removeToastDanger),
  ...
  toast_danger.classList.add("reveal"), (toast_danger.innerText = string);
}
//날짜형식 yyyy-mm-dd
...
function compressImageFile(imgFileDom, previewDom, fromImageName, isMulti) {
  ...
  imageCompression(imageFile, commonImageCompressOptions)
    .then(function (compressedFile) {
      ...
      container.items.add(
        new File([compressedFile], imageFile.name, { type: imageFile.type, lastModified: new Date().getTime() })
      );
...
function multifulModal(current, topModal) {
  ...
  $(".modal-backdrop")
    .eq($(this).length)
    .attr("style", "z-index:" + (parseInt(index) + 1) + "!important");
  ...
  $(objectId + " .required").each(function () {
    ...
    if (!isValid) {
      modal.showAlertModal(
        "done",
        { title: "작성안된 항목이 있습니다.<br>확인 후 작성 하여 주세요.", btn: "확인" },
        function () {}
      );
      $(this).focus();
      return false;
...
function calendarInit(param) {
  ...
  $(`.${tcalendarDays}`)
    .off("click", `.${tcalendarCurrent}`)
    .on("click", `.${tcalendarCurrent}`, function () {
  ...
  function renderCalender(thisMonth) {
    ...
    comAjax.requestSync(
      "GET",
      "/common/service/holiday",
      JSON.stringify({ year: currentYear, month: endDay.getMonth() + 1 }),
      "json",
      function (data) {
        let ret = data.result;
...
function useableTimeOnKeyup() {
  ...
  if (currentIndex == 0 || currentIndex == 4) {
    // time : hour
    var maxVal = 25;
    if (val >= maxVal) this.value = String(maxVal - 1); // change : 24
    ...
    // time : min
    var maxVal = 60;
    if (val >= maxVal) this.value = String(maxVal - 1); // change : 59
